...
import { getTimestamp } from 'src/common/util/timestamp';

@Injectable()
export class WxService {
  private wxApi: AxiosInstance;
  private accessToken = '';
  private accessTokenExpiresAt = 0;

  constructor(private configService: ConfigService) {
    this.wxApi = axios.create({
      baseURL: 'https://api.weixin.qq.com/',
      timeout: 5000,
    });

    this.wxApi.interceptors.response.use((res) => {
  ...
  verifyServer(params: any) {
    const token = this.configService.get('WX_CB_TOKEN');
  ...
  replyAiImageMessage(params: any) {
    ...
    let msg =
      'TALE 专心打造实境沉浸式体验，拓展艺术与科技结合的边界❗\n首个主题【迷津】炸裂开启，神秘实验室招募调查员⚠️⚠️⚠️\n欢迎你的到来！';
    if (bucket && objectId) {
      msg = `<a target="_blank" href="https://style-transfer.nil.work/view?id=${objectId}&bucket=${bucket}">点击此处</a>获取你的任务档案记录`;
    }
    return `
      <xml>
