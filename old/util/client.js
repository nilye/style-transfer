...
proto.authorization = function authorization(
  method,
  resource,
  subres,
  headers
    ...
    return await retry(request.bind(this), this.options.retryMax, {
      errorHandler: (err) => {
        const _errHandle = (_err) => {
          ...
          const requestErrorRetryHandle =
            this.options.requestErrorRetryHandle || (() => true);
...
async function request(params) {
  ...
  if (err) {
    ...
    if (err.name === "ResponseTimeoutError") {
      err.message = `${
        err.message.split(",")[0]
      }, please increase the timeout or use multipartDownload.`;
    }
    throw err;
  }
